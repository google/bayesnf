...
_NF_METHOD = flags.DEFINE_string(
    ...
    'NBEATS',
    help=(
        'Method for NeuralForecast baseline'
...
def run_experiment_bayesnewton(
  ...
  energy = objax.GradValues(model.energy, model.vars())  # pytype: disable=attribute-error

  @objax.Function.with_vars(model.vars() + opt_hypers.vars())
  def train_op():
    # perform inference and update variational params
    model.inference(lr=lr_newton)  # pytype: disable=attribute-error
  ...
  def compute_metrics(model):
    posterior_mean, _ = model.predict_y(X=t_test, R=R_test)
...
def run_experiment_gpflow(
  ...
  # pylint:disable=invalid-name,g-import-not-at-top,g-importing-member
  import GPflow
  ...
  x_probe = np.concatenate((table.x_train, table.x_test))
  y_probe = np.concatenate((table.y_train, table.y_test))
  yhat_mean, yhat_var, _ = _prediction_fn(x_probe, y_probe)
...
def run_experiment_rf(
  ...
  index_probe = np.concatenate((table.index_train, table.index_test))
  x_probe = np.concatenate((table.x_train, table.x_test))
  _ = np.concatenate((table.y_train, table.y_test))
...
def run_experiment_neuralforecast(
  ...
  _ = table.df_train.dropna(subset=target).copy()
  _ = table.df_test.dropna(subset=target).copy()
  ...
  horizon = int(horizon)
  logging.info(f'NeuralForecast {horizon=}')  # pylint: disable=logging-fstring-interpolation
  ...
  static_df = (
      df_train_horizon.groupby(['location'])[static_features]
      .max()
      .reset_index()
  )
